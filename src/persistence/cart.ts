...
export const getCart = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    else throw new Error('No products on cart.');
  } catch (e) {
    res.status(400).json({ error: e.error, message: e.message });
  }
};

export const getCartProduct = async (req: Request, res: Response): Promise<void> => {
  try {
    const product = await getServicesCartProduct(req.params.id);
    if (product) res.json({ data: product });
    else throw new Error('Product does not exist on cart');
  } catch (e) {
    res.status(400).json({ error: e.error, message: e.message });
  }
};

export const saveCartProduct = async (req: Request, res: Response): Promise<void> => {
  try {
